...
class NCSNpp(nn.Module):
    ...
    def add_argparse_args(parser,config):
        ...
        parser.add_argument("--rt_nn", default='t', help="Dropout rate for the backbone.")
    ...
    def __init__(self,
        ...
        **kwargs
        ...
        dose_plus_dropout=kwargs.get('rt_nn', 0.0)
        ...
        if dose_plus_dropout =='t':

            rt_net=[]

            if embedding_type == 'fourier':

                rt_net.append(layerspp.GaussianFourierProjection(
                    embedding_size=nf, scale=fourier_scale
                ))
                embed_dim = 2 * nf
            elif embedding_type == 'positional':
            ...
            rt_net.append(nn.Linear(embed_dim, nf * 4))
            rt_net[-1].weight.data = default_initializer()(rt_net[-1].weight.shape)
            nn.init.zeros_(rt_net[-1].bias)

            rt_net.append(nn.Linear(nf * 4, nf * 4))
            rt_net[-1].weight.data = default_initializer()(rt_net[-1].weight.shape)
            nn.init.zeros_(rt_net[-1].bias)

            rt_net.append(nn.Linear(nf * 4, 1)) 
            rt_net[-1].weight.data = default_initializer()(rt_net[-1].weight.shape)
            nn.init.zeros_(rt_net[-1].bias)

            self.n_new = 1
            self.rt_net = nn.ModuleList(rt_net)

    def get_dropout(self, time_cond):
        rt_net = self.rt_net
        nn_idx = 0
        copy_ncsnpp = True 
        ...
        if copy_ncsnpp:
            rt_net[nn_idx].W.data.copy_(self.all_modules[nn_idx].W.data)

        temb = rt_net[nn_idx](torch.log(used_sigmas))  
        nn_idx += 1
        
        if copy_ncsnpp:
            rt_net[nn_idx].weight.data.copy_(self.all_modules[nn_idx].weight.data)
            rt_net[nn_idx].bias.data.copy_(self.all_modules[nn_idx].bias.data)

        temb = rt_net[nn_idx](temb) 
        nn_idx += 1
        ...
        temb = rt_net[nn_idx](self.act(temb)) 
        nn_idx += 1
