...
def SNRseg(clean_speech, processed_speech,fs, frameLen=0.032, overlap=0.75):
...
def composite(clean_speech, processed_speech, fs):
    wss_dist=wss(clean_speech, processed_speech, fs, frameLen=0.032, overlap=0.75)
    
    # llr_mean=llr(clean_speech, processed_speech, fs, frameLen=0.032, overlap=0.75)
    max_attempts = 10
    attempt = 0
    llr_mean = None
    while attempt < max_attempts:
        try:
            ...
            if attempt == max_attempts:
                print(f"Failed to compute llr")

    segSNR=SNRseg(clean_speech, processed_speech, fs, frameLen=0.032, overlap=0.75)
